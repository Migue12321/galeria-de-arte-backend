...
describe(`GET method for ${modelName}`, () => {
  ...
  test(`Should GET a single ${modelName} item by ID`, async (done) => {
    try {
      await axios.post(route, comment);
      let response = await axios.get(route);
      let currentComment = response.data.find(
        (element) => element.comment === comment.comment
      );
      response = await axios.get(route + "/" + currentComment._id);
      await axios.delete(route + "/" + currentComment._id);
      expect(response.status).toBe(200);
      expect(response.data[0].comment).toEqual(comment.comment);
...
describe(`POST method for ${modelName}`, () => {
  ...
  test(`Should NOT INSERT a new ${modelName} item and reply with status 200 when required fields are empty`, async (done) => {
    try {
      await axios.post(route, invalidComment);
      let response = await axios.get(route);
...
describe(`PUT method for ${modelName}`, () => {
  test(`Should UPDATE the ${modelName} item by ID with the provided data`, async (done) => {
    try {
      ...
      newComment.id = currentComment._id;
      response = await axios.put(route, newComment);
      expect(response.status).toBe(200);
      response = await axios.get(route + "/" + currentComment._id);
      expect(response.data[0].comment).toEqual(newComment.comment);
      await axios.delete(route + "/" + currentComment._id);
      done();
    } catch (error) {
      done.fail(error);
